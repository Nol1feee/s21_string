...
START_TEST(s21_strrchr_one) {
    ...
    ck_assert_str_eq(s21_strrchr(temp, '1'), strrchr(temp, '1'));
}
END_TEST

START_TEST(s21_strrchr_two) {
    char temp[100] = "privet, and you? ";
    ck_assert_str_eq(s21_strrchr(temp, 32), strrchr(temp, 32));
}
END_TEST

START_TEST(s21_strrchr_three) {
    char temp[] = " .      /0 d  ";
    ck_assert_str_eq(s21_strrchr(temp, 100), strrchr(temp, 100));
}
END_TEST
...
START_TEST(s21_strrchr_six) {
    ...
    ck_assert_str_eq(s21_strrchr(temp, 0), strrchr(temp, 0));
}
END_TEST

START_TEST(s21_strrchr_seven) {
    char temp[100] = "elim -> k <- delim ";
    ck_assert_str_eq(s21_strrchr(temp, 33), strrchr(temp, 33));
}
END_TEST
