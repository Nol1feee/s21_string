...
static void flag_i_d(Wid_prec_len* wpl, Flags* flag, char* temp, char* buf,
                     char* result, long int d) {
...
void print_processing(char* buf, int specs, /*int *count_char,*/ va_list* param,
  ...
  if ((specs & spec_d) || (specs & spec_i)) {
    ...
    flag_i_d(wpl, flag, temp, buf, result, d);
  ...
  int s21_sprintf(char* buf, const char* format, ...) {
    ...
    while (*format) {
      ...
      // if we met % for the first time
        ...
        is_spec_start =
            true;  // treats current '%' as start of format specifier
